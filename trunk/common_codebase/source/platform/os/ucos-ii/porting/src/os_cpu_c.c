 ...
 *    pdata :        is a pointer to a user supplied data area that will
 *                   be passed to the task when the task first executes.
 *    ptos  :        is a pointer to the top of stack.
 *                   It is assumed that 'ptos' points to a 'free' entry on
 *                    the task stack.
 *                   If OS_STK_GROWTH is set to 1 then 'ptos' will contain
 *                    the HIGHEST valid address of the stack.
 *                   Similarly, if OS_STK_GROWTH is set to 0, the 'ptos'
 *                    will contains the LOWEST valid address of the stack.
 *    opt   :        specifies options that can be used to alter the behavior
 *                    of OSTaskStkInit(). (see ucos_ii.h for OS_TASK_OPT_...).
 *
 * Returns:
 ...
 *
 * Note(s):
 *    Interrupts are enabled when your task starts executing.
 *    You can change this by setting the PSW to 0x0002 instead.
 *    In this case, interrupts would be disabled upon task startup.
 *    The application code would be responsible for enabling interrupts
 *     at the beginning of the task code.
 *    You will need to modify OSTaskIdle() and OSTaskStat(),
 *     so that they enable interrupts.
...
)
{
    ...
    NO_WARNING(opt);   /* Prevent Compiler warning */
    ...
    *--stk = 1;                        /* r1  */
    *--stk = (INT32U)pdata;            /* r0  */
    *--stk = (SVC_MODE);			   /* cpsr, with INTs enable */
    *--stk = (SVC_MODE);			   /* spsr, with INTs enable */
...
{
    NO_WARNING(ptcb);   /* Prevent Compiler warning */
}
...
*          priority task) and, 'OSTCBCur' points to the task being switched out
